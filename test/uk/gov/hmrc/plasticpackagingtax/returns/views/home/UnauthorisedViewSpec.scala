...
class UnauthorisedViewSpec extends UnitViewSpec with Matchers {
  ...
  "Unauthorised Page view" should {
    ...
    "have proper messages for labels" in {
      ...
      messages must haveTranslationFor("unauthorised.paragraph.1.bullet.1")
      messages must haveTranslationFor("unauthorised.paragraph.1.bullet.1.link")
      messages must haveTranslationFor("unauthorised.paragraph.1.bullet.2.link")
    ...
    "display page paragraph" in {
      view.getElementsByTag("p").get(1) must containMessage("unauthorised.paragraph.1")
    }

    "display get UTR link" in {
      val link = view.getElementById("get_utr_link")
    ...
    "display access PPT link" in {
      val link = view.getElementById("access_ppt_link")
    ...
    "display check PPT application status link" in {
      val link = view.getElementById("check_ppt_application_status_link")
